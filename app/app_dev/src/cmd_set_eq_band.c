...
int do_set_eq_band (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	DSP_IOCTL_2_PARAMS(&leftChanelDsp , IOCTL_BIQUAD_FILTER_band_biquads_set_params, argv[1] , argv[2] );
#else
	...
	DSP_IOCTL_1_PARAMS(&leftChanelEQ , IOCTL_BIQUAD_FILTER_GET_BAND_BIQUADS, &band_set );
	...
	DSP_IOCTL_1_PARAMS(&leftChanelEQ , IOCTL_BIQUAD_FILTER_SET_BAND_BIQUADS, &band_set );
	DSP_IOCTL_1_PARAMS(&rightChanelEQ , IOCTL_BIQUAD_FILTER_SET_BAND_BIQUADS, &band_set );
